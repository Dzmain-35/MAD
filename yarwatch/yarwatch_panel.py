...
class YarWatchPanel(ctk.CTkFrame):
    def __init__(self, master, feature_extractor, logger, output_queue, monitor=None,dashboard_gui=None):
        ...
        self.configure(fg_color="#0E2536")
    ...
    def build_layout(self):
        ...
        self.title_button = customtkinter.CTkButton(self, text="YarWatch", font=("Roboto", 40), fg_color=btn_color, hover_color=hover_color, height=65, command=self.toggle_watch)
        self.title_button.grid(row=1, column=1, padx=20, pady=(20, 15), sticky="nsew")

        self.scan_file_button = customtkinter.CTkButton(self, text="Scan File", font=font, fg_color=btn_color, hover_color=hover_color, height=65, command=self.scan_file)
        self.scan_file_button.grid(row=1, column=0, padx=20, pady=(20, 15), sticky="nsew")

        self.scan_pid_button = customtkinter.CTkButton(self, text="Scan PID", font=font, fg_color=btn_color, hover_color=hover_color, height=65, command=self.scan_pid)
        self.scan_pid_button.grid(row=1, column=2, padx=20, pady=(20, 15), sticky="nsew")

        self.running_processes_label = customtkinter.CTkLabel(self,text="Running Processes",font=font, fg_color=btn_color, height=65)
        self.running_processes_label.grid(row=4, column = 1,padx=20, pady=(20, 15), sticky="nsew")

        self.process_panel = customtkinter.CTkFrame(self, fg_color="#0E2536")
        ...
        self.scroll_border_frame = customtkinter.CTkFrame(self, border_width=10, border_color="white", corner_radius=10, fg_color="transparent")
    ...
    def open_processs_button(self):
        if self.process_panel_visible:
            ...
            # self.geometry removedf"922x{self.default_height}")
            self.process_panel_visible = False
        ...
        while not self.stop_process_thread:
            ...
            for pid in new_pids:
                try:
                    ...
                    self.after(0, self._add_embedded_process, name, pid)
    ...
    def _add_embedded_process(self, name, pid):
        ...
        if self.winfo_exists() and self.process_tree.winfo_exists():
    ...
    def update_output(self, message):
        ...
        if "[Monitor] New process:" in message and self.process_panel_visible:
            try:
                ...
                self.after(0, self._add_process_to_listbox, process_info)
            except Exception:
                pass
    ...
    def process_output_queue(self):
        while not self.output_queue.empty():
            ...
            if isinstance(msg, dict):
                self.update_collapsible_output(msg)
            # Skip plain [INFO] or log lines during Watch mode
