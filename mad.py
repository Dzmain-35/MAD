"""
Forensic Analysis Tool - GUI Module - FIXED VERSION
Main interface - handles UI only, delegates logic to other modules
...
class ForensicAnalysisGUI:
    ...
    def create_processes_subtab(self):
        ...
        self.process_tree.tag_configure('system', background='#1a4d2e', foreground='white')  # Green for system
        self.process_tree.tag_configure('new', background='#8B7500', foreground='white')  # Yellow for new processes
...
File Size: {file_info['file_size']} bytes"""
    ...
    def refresh_process_list(self):
        ...
        # Track existing PIDs before refresh to identify new ones
        existing_pids = set()
        for item in self.process_tree.get_children():
            try:
                values = self.process_tree.item(item, 'values')
                if values and len(values) > 0:
                    existing_pids.add(int(values[0]))
            except:
                pass
        ...
        # Known system/safe process names
        system_process_names = {
            'system', 'registry', 'smss.exe', 'csrss.exe', 'wininit.exe',
            'services.exe', 'lsass.exe', 'svchost.exe', 'winlogon.exe',
            'fontdrvhost.exe', 'dwm.exe', 'audiodg.exe', 'lsm.exe',
            'spoolsv.exe', 'conhost.exe', 'runtimebroker.exe', 'taskhostw.exe',
            'explorer.exe', 'sihost.exe', 'ctfmon.exe', 'searchindexer.exe',
            'wudfhost.exe', 'dllhost.exe', 'backgroundtaskhost.exe'
        }
        ...
        def add_process_tree(proc, parent_id=""):
            ...
            # Determine YARA match status and tags
            yara_status = "No"
            tags = []
            
            # Priority 1: Check for threats (highest priority - RED)
            ...
            # Priority 2: Check if system/safe process (GREEN)
            elif name.lower() in system_process_names or proc.get('scan_skipped'):
                tags.append('system')
            
            # Priority 3: Check if new process (YELLOW)
            elif pid not in existing_pids and self.process_monitor_active:
                tags.append('new')
...
"""
        ...
        # Tab switching
        tabs = {"info": info_frame, "strings": strings_frame}
        buttons = {"info": btn_info, "strings": btn_strings}
