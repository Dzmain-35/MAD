from yarwatch.bootstrap import ensure_requirements
ensure_requirements()
...
class MalwareDashboard(ctk.CTk):
    ...
    #Analyst Notes Section
        notes_outer = ctk.CTkFrame(frame, fg_color="#0E2536")
        notes_outer.pack(fill="both", expand=False, padx=20, pady=(20, 10))

        notes_title = ctk.CTkLabel(notes_outer, text="Analyst Notes", font=("Arial", 22, "bold"), text_color="white")
        notes_title.pack(anchor="w", padx=10, pady=(10, 6))

        self.notes_view = ctk.CTkTextbox(notes_outer, wrap="word", height=160, font=("Consolas", 16),
                                        text_color="white", fg_color="#0A1B27")
        self.notes_view.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        self.notes_view.configure(state="disabled")

        input_row = ctk.CTkFrame(notes_outer, fg_color="#0E2536")
        input_row.pack(fill="x", padx=10, pady=(0, 10))

        self.note_input = ctk.CTkTextbox(input_row, wrap="word", height=90, font=("Consolas", 16),
                                        text_color="white", fg_color="#0A1B27")
        self.note_input.pack(side="left", fill="both", expand=True, padx=(0, 10))

        save_btn = ctk.CTkButton(input_row, text="Save Note", fg_color="#A30000", hover_color="#8A0000",
    ...
    def _format_notes(self, notes):
        ...
        for n in notes:
            ts = n.get("timestamp", "")
            au = n.get("author", "Analyst")
            tx = n.get("text", "")
            lines.append(f"[{ts}] {au}:\n{tx}\n")
        return "\n".join(lines)

    def _refresh_notes_view(self):
        case = self.case_manager.get_current_case()
        notes = (case or {}).get("notes", [])
        if hasattr(self, "notes_view") and self.notes_view and self.notes_view.winfo_exists():
            ...
            self.notes_view.insert("end", self._format_notes(notes))
            self.notes_view.configure(state="disabled")
    ...
    def _on_save_note(self):
        ...
        if not text:
            messagebox.showinfo("Empty", "Type a note before saving.")
            return

        author = case.get("analyst") or "Analyst"
        ok = self.case_manager.add_note_to_current_case(text, author)
        if ok:
            self.note_input.delete("1.0", "end")
            self._refresh_notes_view()
            messagebox.showinfo("Saved", "Note added to case.")
        else:
    ...
    def _process_new_case_upload(self, file_path, analyst, report_url, sha256):
        ...
        try:
            # --- Copy sample into case store (no silent pass) ---
            ...
            if os.path.exists(dest_path):
                ...
                while os.path.exists(dest_path):
                    dest_path = os.path.join(destination_path, f"{name} ({i}){ext}")
                    i += 1

            shutil.copyfile(file_path, dest_path)   # raise if it fails
            ui(row.set_progress, 0.2)

            # --- Run scan against the stored copy & capture result ---
            ui(row.set_status, "YARA scanning…")
            ui(row.start_indeterminate)

            extractor = FeatureExtractor("C:\\Users\\REM\\Desktop\\Mal_Data")
            logger = YarLogger(gui_queue=self.analysis_output_queue)
            ordered = run_yara_file(dest_path, self, extractor, logger)
...
class StringsViewer(ctk.CTkToplevel):
    def __init__(self, master, pid: int):
        ...
        self._quick   = tk.BooleanVar(value=True) 
